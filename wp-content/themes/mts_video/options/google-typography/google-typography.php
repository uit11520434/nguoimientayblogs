...
class mtsGoogleTypography {
	...
	function help_tab() {
		...
		$adding_title					= __('Adding A Collection', 'video' );
		$adding_content				= '<p>'.__('To add a new font for use on your site. Click the "Add New" button on the top left of the page near the "Google Typography" title.', 'video' ).'</p>';
		$adding_content			 .= '<p>'.__('Once added, a new font row will appear on the page below. Next you can continue to customize your font (more info in the "Customizing" help tab).', 'video' ).'</p>';
		//$adding_content			 .= '<p><a href="https://vimeo.com/67957799" target="_blank">'.__('Watch The Video Tutorial &rarr;', 'video' ).'</a></p>';
		$customizing_title 		= __('Customizing', 'video' );
		$customizing_content	= '<p>'.__('Customizing fonts is easy; after adding a new font row you can then customize the following font attributes:', 'video' ).'</p>';
		$customizing_content .= '<ul><li><b>'.__('Preview Text', 'video' ).'</b> - '.__('Used for live previewing your changes. This text does not appear anywhere on your website.', 'video' ).'</li><li><b>Preview Background Color</b> - Allows you to swap between light and dark backgrounds when previewing this font.</li><li><b>'.__('Font Family', 'video' ).'</b> - '.__('The font family to use for this font. Choose from a real-time list of all available Google Fonts.', 'video' ).'</li><li><b>'.__('Font Variant', 'video' ).'</b> - '.__('The variant to use for this font. Note: Each font has it\'s own variant options.', 'video' ).'</li><li><b>'.__('Font Size', 'video' ).'</b> - '.__('The size you would like this font to be.', 'video' ).'</li><li><b>'.__('Font Color', 'video' ).'</b> - '.__('The color you\'d like to use for this font.', 'video' ).'</li><li><b>'.__('CSS Selectors', 'video' ).'</b> - '.__('The HTML tags or CSS selectors you\'d like this font to apply to (more info in the "CSS Selectors" help tab). You can specify multiple selectors separated by comma\'s. Ex: h1, #some_id, .some_class', 'video' ).'</li></ul>';
		$selectors_title 			= __('CSS Selectors', 'video' );
		$selectors_content		= '<p>' . __('CSS Selectors are used to hook your font rows into your actual website. Once you\'ve added, customized, and defined CSS selectors for your fonts, Google Typography will automatically insert all the necessary CSS into your website.', 'video' ) . '</p>';
		$selectors_content		= '<p>' . __('Here are some examples of the selectors you can use:', 'video' ) . '</p>';
		$selectors_content	 .= '<ul><li><b>'.__('IDs', 'video' ).':</b> '.__('#selector', 'video' ).'</li><li><b>'.__('Classes:', 'video' ).':</b> '.__('.selector', 'video' ).'</li><li><b>'.__('HTML Tags', 'video' ).':</b> '.__('span', 'video' ).'</ul>';
		$selectors_content	 .= '<p><b>'.__('Example', 'video' ).':</b> '.__('#selector span.date', 'video' ).'</p>';
	...
	function plugin_link($links, $file) {
		if ( $file == 'google-typography/google-typography.php' ) {
			$links['settings'] = sprintf( '<a href="%s"> %s </a>', admin_url( 'themes.php?page=typography' ), __( 'Settings', 'video' ) );
		}
		return $links;
	}
	...
	function options_ui() {
		...
		$title								= __('Theme Typography', 'video' );
		$loading							= __('Loading Your Collections', 'video' );
		$add_new        			= __('Add New Collection', 'video' );
		$reset        				= __('Reset Collections', 'video' );
		$preview_text   			= __('Type in some text to preview...', 'video' );
		$collection_title 			= __('New Collection', 'video' );
        //$this->preview_text = $preview_text; // to pass it to JS
		$preview_hint   			= __('Preview Background Color', 'video' );
		//$font_family_title  	= __('Font family...', 'video' );
        $std_font_family_title  	= __('Standard Fonts', 'video' );
        $google_font_family_title  	= __('Google Fonts', 'video' );
		$font_variant_title 	= __('Variant...', 'video' );
		$font_size_title 			= __('Size...', 'video' );
        $backup_font_title          = __('Fallback:', 'video' );
		$css_selectors_title	= __('CSS Selectors:', 'video' );
        $additional_css_title	= __('Additional CSS:', 'video' );
		$delete_button_text   = __('Delete', 'video' );
		$save_button_text			= __('Save', 'video' );
		
		$welcome_title 				= __('Theme Typography', 'video' );
		$welcome_subtitle			= __('Get started in 3 steps.', 'video' );//.'<a href="https://vimeo.com/67957799" target="_blank">'.__('Watch the video tutorial &#x2192;', 'video' ).'</a>';
		$step_1_title 				= __('1. Pick A Font', 'video' );
		$step_1_desc					= __('Choose from any of the 600+ Google Fonts.', 'video' );
		$step_2_title 				= __('2. Customize It', 'video' );
		$step_2_desc					= __('Pick a size, variant, color and more.', 'video' );
		$step_3_title 				= __('3. Attach It', 'video' );
		$step_3_desc					= __('Attach your font to any CSS selector(s).', 'video' );
		$year									= date("Y");
        $themename = MTS_THEME_NAME;
        
        $character_sets_title = __('Character sets', 'video' );
        $character_sets_desc = __('Choose the character sets you wish to include. Please note that not all sets are available for all fonts.', 'video' );
        ...
        $character_sets = array(
            'latin' => __('Latin', 'video' ),
            'latin-ext' => __('Latin Extended', 'video' ),
            'cyrillic' => __('Cyrillic', 'video' ),
            'cyrillic-ext' => __('Cyrillic Extended', 'video' ),
            'greek' => __('Greek', 'video' ),
            'greek-ext' => __('Greek Extended', 'video' ),
            'vietnamese' => __('Vietnamese', 'video' ),
            'khmer' => __('Khmer', 'video' ),
            'devanagari' => __('Devanagari', 'video' ),
        );
...
EOT;
	...
	function admin_scripts() {
		...
		wp_localize_script('google-typography', 'googletypography', array(
	            'reset_confirm' => __("Are you sure you want to revert to the theme's default collections? Note: You will lose any custom collections you've created.", 'video' ),
	            'delete_confirm' => __("Are you sure you want to delete this collection?", 'video' )
	        ));
